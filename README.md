...
flowchart TD
    ...
    subgraph Ingestion["services/ingestion"]
        DocToMd["document_to_markdown"]
    end

    subgraph Knowledge["services/knowledge"]
        KnowledgeMgr["knowledge_manager"]
    end

    subgraph Inference["services/inference"]
        LLMService["llm_inference_service"]
    end

    subgraph Experiment["services/experiment"]
        ExpMgr["experiment_manager"]
    end
    ...
    LLM["Local LLM / OpenAI API"]

    User -->|操作| Streamlit

    Streamlit -->|ドキュメントアップロード| DocToMd
    DocToMd -->|markdown 変換| KnowledgeData

    Streamlit -->|ナレッジ管理| KnowledgeMgr
    KnowledgeMgr <-->|読み書き| KnowledgeData

    Streamlit -->|推論リクエスト| LLMService
    LLMService -->|ランダム選択| KnowledgeData
    LLMService -->|プロンプト読み込み| PromptData
    LLMService -->|推論呼び出し × N| LLM
    LLM -->|JSON 応答 × N| LLMService
    LLMService -->|アンサンブル集約結果| Streamlit
    LLMService -->|ログ保存| LogData

    Streamlit -->|実験管理| ExpMgr
    ExpMgr <-->|読み書き| ExperimentData
    ExpMgr -->|実験設定で推論| LLMService
```
